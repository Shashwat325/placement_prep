...
// Which field to speak in "Listen and Speak" mode, per question type.
// repeat_paragraph: the actual sentence to repeat lives in correct_answer.
// summarize_paragraph / jumbled_sentence: the content lives in prompt.
...
// % of reference words that also appear in the transcript (order-independent).
...
export default function TakeTest() {
  ...
  // null until the user picks a mode; 'listen' | 'read' after that
  ...
  const [result, setResult] = useState(null); // set once the test is completed
  ...
  // On mount: start an attempt and fetch a batch of questions for this topic.
  useEffect(() => {
  ...
  const startRecording = async () => {
    try {
      ...
      setRecordingTime(0);
      setQuestionScores(null); // clear any previous attempt's score for this question
  ...
  // Send audio to voice scoring service
  ...
  // Handle text input change (for transcript or jumbled sentence answer)
  ...
  // Mode selection screen — shown once, before the first question
                  ...
                  {recording && (
                    <>
                      <button
                        ...
                        disabled={submitting}
                        className="btn btn-danger me-2 px-4 py-2"
                      >
                        Stop Recording
                      </button>
              ...
              {questionScores && !isProcessing && (
                <div className="score-breakdown" style={{ marginTop: '1rem', padding: '1rem', border: '1px solid #ddd', borderRadius: '8px' }}>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                    <li className="text-dark">🗣️ Pronunciation: {questionScores.pronunciation}%</li>
                    <li className="text-dark">🌊 Fluency: {questionScores.fluency}%</li>
                    <li className="text-dark">
                      ⏱️ Speed: {questionScores.speedWpm} wpm ({questionScores.speedScore}%)
                    </li>
                    {currentQuestion.question_type !== 'summarize_paragraph' && (
